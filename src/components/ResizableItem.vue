<template>
    <div ref="item" :class="['rv-container', 'rv-rect', 'rv-scale']" :style="itemStyle">
        ...
        <div ref="slideBar" :class="['rv-rect']" :style="slideBarStyle" @mouseenter="onMouseEnter"
...
export default {
    ...
    methods: {
        ...
        onMouseOut() {
            ...
            this.$emit('item-resize', this.$refs.item.clientWidth, this.$refs.item.clientHeight, this.$refs.item)
        },
        getSize() {
            return { width: this.$refs.item.clientWidth, height: this.$refs.item.clientHeight }
        }
    }
}
</script>
